...
class NoteBloc extends Bloc<NoteEvent, NoteState> {
  ...
  Future<void> _onGetArchivedNotes(GetArchivedNotes event, Emitter<NoteState> emit) async {
  ...
  Future<void> _onCreateNote(CreateNote event, Emitter<NoteState> emit) async {
    ...
    try {
      final note = await noteRepository.createNote(title: event.title, body: event.body);
  ...
  Future<void> _onGetNoteById(GetNoteById event, Emitter<NoteState> emit) async {
  ...
  Future<void> _onArchiveNote(ArchiveNote event, Emitter<NoteState> emit) async {
    emit(state.copyWith(actionState: const Loading()));
    try {
  ...
  Future<void> _onUnarchiveNote(UnarchiveNote event, Emitter<NoteState> emit) async {
    emit(state.copyWith(actionState: const Loading()));
    try {
